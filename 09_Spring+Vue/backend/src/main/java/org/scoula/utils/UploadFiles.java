...
//        response.setHeader("Content-Disposition", "inline;filename=\"" + filename + "\"");
    ...
    public static void downloadImage(HttpServletResponse response, File file) {
        try {
            ...
            String mimeType = Files.probeContentType(path); // 파일 확장자로 MIME TYPE 추정
            log.info("mimeType : {}", mimeType);
            // 응답 Content-Type을 파일 타입으로 지정하면 브라우저가 이미지를 바로 렌더링
            ...
            try (
                ...
                Files.copy(path,bos); // 서버 파일용 응답 스트림으로 복사 (전송)
            }
        } catch (IOException e) {
